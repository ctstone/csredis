...
namespace ctstone.Redis
{
    public partial class RedisSentinelClient
    {
        ...
        {
            return WriteAsync(RedisSentinelCommand.Masters());
        }

        public Task<RedisMasterInfo> MasterAsync(string masterName)
        {
            return WriteAsync(RedisSentinelCommand.Master(masterName));
        }
        ...
        {
            return WriteAsync(RedisSentinelCommand.Sentinels(masterName));
        }
        ...
        {
            return WriteAsync(RedisSentinelCommand.Slaves(masterName));
        }
        ...
        {
            return WriteAsync(RedisSentinelCommand.GetMasterAddrByName(masterName));
        }

        public Task<RedisMasterState> IsMasterDownByAddrAsync(string ip, int port, long currentEpoch, string runId)
        {
            return WriteAsync(RedisSentinelCommand.IsMasterDownByAddr(ip, port, currentEpoch, runId));
        }

        public Task<long> ResetAsync(string pattern)
        {
            return WriteAsync(RedisSentinelCommand.Reset(pattern));
        }

        public Task<string> FailoverAsync(string masterName)
        {
            return WriteAsync(RedisSentinelCommand.Failover(masterName));
        }

        public Task<string> MonitorAsync(string name, int port, int quorum)
        {
            return WriteAsync(RedisSentinelCommand.Monitor(name, port, quorum));
        }

        public Task<string> RemoveAsync(string name)
        {
            return WriteAsync(RedisSentinelCommand.Remove(name));
        }

        public Task<string> SetAsync(string masterName, string option, string value)
        {
            return WriteAsync(RedisSentinelCommand.Set(masterName, option, value));
        }
        #endregion
    }
}
