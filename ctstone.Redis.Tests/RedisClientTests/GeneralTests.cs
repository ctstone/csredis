    ...
    public class GeneralTests : RedisTestBase
    {
        ...
        public void TestBuffering()
        {
            ...
            using (new RedisTestKeys(_redis, "test1"))
            {
                _redis.Set("test1", data);
                {
                    _redis.BufferFor(x => x.Get("test1"));
                    byte[] buffer = new byte[64];
                    int bytes_read;
                    StringBuilder sb = new StringBuilder();
                    while ((bytes_read = _redis.Read(buffer, 0, buffer.Length)) > 0)
                    {
                ...
                _redis.Set("test1", data);
                {
                    ...
                    int bytes_read;
                    StringBuilder sb = new StringBuilder();
                    while ((bytes_read = _redis.Read(buffer, 5, buffer.Length / 2)) > 0)
                    {
